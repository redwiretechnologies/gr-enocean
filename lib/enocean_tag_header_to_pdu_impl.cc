...
enocean_tag_header_to_pdu::sptr enocean_tag_header_to_pdu::make(pmt::pmt_t tag_name)
{
    return gnuradio::make_block_sptr<enocean_tag_header_to_pdu_impl>(tag_name);
}
...
void enocean_tag_header_to_pdu_impl::publish_data_packet()
{
    ...
    pdu_meta = pmt::dict_add(
        pdu_meta, pmt::intern("packet_length"), pmt::from_long(d_packet_length));
...
int enocean_tag_header_to_pdu_impl::work(int noutput_items,
                                         ...
                                         gr_vector_void_star& output_items)
{
    ...
    // Make sure we have at least 5 bytes, 40 input items includig the sob, unless we are
    ...
    if (!d_in_packet && !tag_found) // we are not in a packet, no packet  this is first
                                    // because it is the most common state
    {
        ...
        if (d_data_to_copy < (abs_end - d_tags[0].offset)) {
            ...
            consumed = d_data_to_copy;
            publish_data_packet();
        } else { // Not enough data to close the previous packet, delete it and the data
                 // up to the tag
        ...
        if (abs_end - d_tags[0].offset > 8) {
            ...
            d_data.insert(d_data.end(),
                          &in[d_tags[0].offset - abs_start],
                          &in[d_tags[0].offset - abs_start + 8]);
            d_packet_length <<= 1;
            uint8_t bytes = 0x00;
            ...
            d_data_to_copy =
                d_packet_length * 8; // the data should be in bytes but we have bits
